...
class LinearChainCRF():
	...
	def train(self, corpus_filename, model_filename):
		...
		# 원본:self.feature_set.scan(self.training_data)
		# 피쳐 생성을 배치 단위로 할 수 있도록 
	...
	# with batch
	def train(self, corpus_filename, model_filename,iteration):
		"""
		Estimates parameters using conjugate gradient methods.(L-BFGS-B used)
		"""
		start_time = time.time()
		...
		for iter in range(iteration):
			...
			# 원본:self.feature_set.scan(self.training_data)
			# 피쳐 생성을 배치 단위로 할 수 있도록 
			self.feature_set.scan(self.training_data,ff)
			exit()
			self.label_dic, self.label_array = self.feature_set.get_labels()
			self.num_labels = len(self.label_array)
		...
		# have to modify save_model feature part to connect to feature file with is ff 
		self.save_model(model_filename)
...
def batch_testcode():
	...
	CRF = LinearChainCRF()
	ff = CRF_batch.feature_file()
	ff.write_feature_str_with_value('엄',-1,0,0)
	ff.write_feature_str_with_value('찬',-1,1,1)
	
	#ff.write_only_value('엄',-2,0,2)

	exit()
	input_filename = "mini_Sejong_test.txt"
	model_name = "test_batch.model"
	iteration = 4
	CRF.train(input_filename,model_name,iteration)
